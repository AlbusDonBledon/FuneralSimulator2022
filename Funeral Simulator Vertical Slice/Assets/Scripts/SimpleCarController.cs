...
public class SimpleCarController : MonoBehaviour
{
    ...
    string[] joysticsConnected;

    private int inputType = 17; //переключать с жойстика на клаву при значении > or < 0
	...
	void Update ()
    {
        ...
        //joysticsConnected=Input.GetJoystickNames();
        //print("joystics connected : " + joysticsConnected.Length);
        //if (joysticsConnected.Length > 0){ inputType = 1; }
        //else { inputType = -1; }

        GetInput();
        if (Input.GetKeyDown(KeyCode.O))// || Input.GetButton("DriftJoy"))
        {
            inputType = inputType * (-1);
        }
    }
    ...
    {     
        if (Application.platform == RuntimePlatform.WindowsEditor)
        {
            if (inputType > 1)
            {
                ...
                {
                driveInput = Input.GetAxis("VerticalJoy") *-1;
            }
